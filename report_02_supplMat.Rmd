...
author: 
  - Marc H. Taylor, Holger Haslob, Kim Hünerlage, Karl Michael Werner, and Alexander Kempf
...
**Table S`r iTab=iTab+1; iTab`.** Cross-validation performance metrics for each biomass size fraction model. Stars indicate the best performance of each metric by model.
...
**Table S`r iTab=iTab+1; iTab`.** Best model parameter summary for the `r fnameAppendix` size fraction model.
...
tmp2 <- merge(data.frame(Component = "B. random parameters"), tmp2)
...
The call to the GLM describing biomass as a function of catch per unit effort (CPUE) was as follows:
