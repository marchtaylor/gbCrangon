...
Spatial mesh used by the SPDE (stochastic partial differential equation) based approach is shown in Figure S`r iFig+1`. Before mesh creation, latitude and longitude coordinates were converted to equidistant projection (UTM) coordinates to ensure constant distance units across the study area. A maximum edge length of 20 km was used for the interior mesh area. An exterior barrier was defined using a spatial polygon defining the coastline, major islands, and an outer limit that approximated the furthest offshore extent of Demersal Young Fish Survey (DYFS) sample locations.
...
**Table S`r iTab=iTab+1; iTab`.** Best model (mod3_st_ar1) parameter summary for the `r fnameAppendix` size fraction model.
...
```{r, echo=FALSE}
fname <- file.path("output/obs_vs_pred_by_fraction.png")
tmp <- readPNG(fname)
knitr::include_graphics(fname, dpi = floor(dim(tmp)[2]/6))
```

**Figure S`r iFig=iFig+1; iFig`.** Plots of observed versus predicted values (binned) from the best fitting model by size fraction. Colors indicate the number of  aggregated samples within discrete x-y bins. The dashed grey line indicates the 1:1 identity line for reference. As the values are shown on a log-scale, zero-value observations are not shown (n = 17 of 8058, ~0.2\% of total samples).
...
### Prediction to model domain and calculation of total biomass index

Figures S`r paste0(iFig+1, "-", iFig+2)` show the predicted biomass by year for the small and large size fractions. A comparison of the summation of yearly indices from the small and large biomass fraction models is compared to the single index of combined biomass model in Figure S`r iFig+3`
...
**Figure S`r iFig=iFig+1; iFig`.** Prediction to grid by year for the small biomass model. Points indicate location of survey samples for each year. The total biomass estimate of each year is shown in the lower-right corner of each map.
...
**Figure S`r iFig=iFig+1; iFig`.** Prediction to grid by year for the large biomass model. Points indicate location of survey samples for each year. The total biomass estimate of each year is shown in the lower-right corner of each map.
...
Spatial patterns in the coefficient of variation (sd/mean) of yearly predicted biomass in the combined size fraction model are shown in Figure S`r paste0(iFig+1)`. As expected, the patterns reflect distance to observed biomass positions, with relatively higher uncertainties in less-sampled deeper areas as well as some nearshore estuaries.  

```{r, echo=FALSE}
fname <- file.path("output/cv_maps_combined.png")
tmp <- readPNG(fname)
knitr::include_graphics(fname, dpi = floor(dim(tmp)[2]/6))
```

**Figure S`r iFig=iFig+1; iFig`.** Coefficient of variation (sd/mean) for predicted biomass by year in the combined size fraction model. 
...
**Table S`r iTab=iTab+1; iTab`.** Model of LPUE as a function of biomass. For each model, based on unique time period and size fraction combinations, the relationship coefficients, confidence intervals (95%), relationship shape type, and $R^2$ are presented. 
...
**Table S`r iTab=iTab+1; iTab`.** GAM model summary of discard rate as a function of year (annual term) and fractional year (seasonal term). 
...
The resulting mean monthly average discard rates used for the pre-data historical period (2000-2012) ranged between  `r drSumLab`.
...
**Table S`r iTab=iTab+1; iTab`.** Model of CPUE as a function of the combines biomass estimate. The relationship coefficient estimates and confidence interval (95%), relationship shape type and $R^2$ are presented.
...
**Figure S`r iFig=iFig+1; iFig`.** Combined biomass estimate versus catch per unit effort (CPUE). Regression is shown by the dark grey line with shaded area showing the confidence intervals (95%). The $R^2$ of the relationship is indicated at the top-left.
...
The inverse relationship, with the combined biomass estimate as the response variable and CPUE as the explanatory variable was used to predict monthly exploitable biomass in the entire time series. The call to the GLM was as follows:
